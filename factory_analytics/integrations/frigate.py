...
class FrigateClient:
    def __init__(self, settings: dict[str, Any]):
        ...
        self.base_url = (settings.get('frigate_url') or '').rstrip('/')
    ...
    def health(self) -> dict[str, Any]:
        ...
        try:
            with httpx.Client(timeout=15, verify=self.verify_tls, headers=self._headers(), auth=self._auth()) as client:
    ...
    def fetch_cameras(self) -> list[str]:
        if not self.base_url:
            return []
        try:
            with httpx.Client(timeout=20, verify=self.verify_tls, headers=self._headers(), auth=self._auth()) as client:
    ...
    def fetch_latest_snapshot(self, camera_name: str, destination: Path) -> Path:
        ...
        endpoints = [
            ...
            f'{self.base_url}/api/{camera_name}',
        ]
        last_error = None
        for endpoint in endpoints:
            try:
                with httpx.Client(timeout=30, verify=self.verify_tls, headers=self._headers(), auth=self._auth()) as client:
                ...
                last_error = exc
        raise RuntimeError(f'Failed to fetch latest snapshot for {camera_name}: {last_error}')
