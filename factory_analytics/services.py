...
class AnalyticsService:
    ...
    def update_settings(self, payload: dict[str, Any], actor: str = 'api') -> dict[str, Any]:
    ...
    def sync_cameras_from_frigate(self) -> dict[str, Any]:
        ...
        self.db.log_audit('api', 'frigate.sync_cameras', 'camera', payload={'count': len(synced)})
    ...
    def system_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            db_msg = 'ok'
        return {'ok': db_ok and bool(frigate.get('ok')) and bool(ollama.get('ok')), 'app': {'ok': True}, 'database': {'ok': db_ok, 'message': db_msg}, 'frigate': frigate, 'ollama': ollama}
    ...
    def queue_analysis(self, camera_id: int, payload: dict[str, Any] | None = None):
        job = self.db.schedule_job(camera_id, payload=payload or {})
        self.db.log_audit('api', 'job.schedule', 'job', str(job['id']), payload or {})
        return job
    ...
    def process_one_pending_job(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            segment = self.db.create_segment(job_id=job['id'], camera_id=camera['id'], start_ts=start_ts, end_ts=end_ts, label=result.get('label', 'uncertain'), confidence=float(result.get('confidence', 0.0)), notes=result.get('notes', ''), evidence_path=str(snapshot_path.relative_to(DATA_ROOT.parent)))
            day = start_ts[:10]
            seconds = max(1, int((datetime.fromisoformat(end_ts) - datetime.fromisoformat(start_ts)).total_seconds()))
            self.db.update_daily_rollup(day, camera['id'], segment['label'], seconds)
            self.db.update_camera(camera['id'], {'last_run_at': datetime.now(timezone.utc).isoformat(), 'last_status': 'ok'})
            self.db.mark_job_finished(job['id'], 'success', raw_result=result, snapshot_path=str(snapshot_path.relative_to(DATA_ROOT.parent)))
            return {'job': self.db.get_job(job['id']), 'segment': segment}
        except Exception as exc:
            logger.exception('Job processing failed')
            self.db.update_camera(camera['id'], {'last_run_at': datetime.now(timezone.utc).isoformat(), 'last_status': f'error: {exc}'})
            self.db.mark_job_finished(job['id'], 'failed', error=str(exc))
            return {'job': self.db.get_job(job['id']), 'error': str(exc)}

    def _resolve_job_window(self, job: dict[str, Any], camera: dict[str, Any], settings: dict[str, Any]):
    ...
    def review_segment(self, segment_id: int, reviewed_label: str, review_note: str, review_by: str = 'operator'):
        result = self.db.review_segment(segment_id, reviewed_label, review_note, review_by)
        self.db.log_audit(review_by, 'segment.review', 'segment', str(segment_id), {'reviewed_label': reviewed_label, 'review_note': review_note})
        return result
