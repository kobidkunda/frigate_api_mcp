...
async function saveSettings(ev){ ev.preventDefault(); const fd=new FormData(ev.target); const values=Object.fromEntries(fd.entries()); values.frigate_verify_tls = values.frigate_verify_tls === 'true'; values.scheduler_enabled = values.scheduler_enabled === 'true'; values.llm_enabled = values.llm_enabled === 'true'; values.analysis_interval_seconds = Number(values.analysis_interval_seconds || 300); values.llm_timeout_sec = Number(values.llm_timeout_sec || 120); await api('/api/settings',{method:'PUT',body:JSON.stringify({values})}); await refreshAll(); alert('Settings saved'); }
...
async function loadSegments(){ const segments=await api('/api/history/segments'); const el = document.getElementById('segmentsTable'); if(!el) return; el.innerHTML = `<table><thead><tr><th>ID</th><th>Camera</th><th>Label</th><th>Confidence</th><th>Window</th><th>Evidence</th><th>Review</th></tr></thead><tbody>${segments.map(s=>{ const evidenceFrames = Array.isArray(s.evidence_frames) ? s.evidence_frames.filter(Boolean) : []; const primaryEvidence = evidenceFrames[0] || s.evidence_path; return `<tr><td>${s.id}</td><td>${s.camera_name}</td><td>${s.reviewed_label || s.label}</td><td>${Number(s.confidence || 0).toFixed(2)}</td><td>${fmtTs(s.start_ts)}<div class="small">${fmtTs(s.end_ts)}</div></td><td>${primaryEvidence ? `<img src="/${primaryEvidence}" alt="evidence" loading="lazy" style="max-width:160px;max-height:120px;border:1px solid #222;border-radius:4px" onerror="this.replaceWith(document.createTextNode('no image'))" />${evidenceFrames.length > 1 ? `<div class="text-xs text-outline">${evidenceFrames.length} frames</div>` : ''}` : ''}</td><td><div class="inline-actions"><button class="secondary" onclick="reviewSegment(${s.id}, 'working')">Mark working</button><button class="secondary" onclick="reviewSegment(${s.id}, 'not_working')">Mark not working</button><button class="secondary" onclick="reviewSegment(${s.id}, 'no_person')">Mark no person</button></div></td></tr>`}).join('')}</tbody></table>`; }
